const DEFAULTS = { baseUrl: 'https://hr.sftntx.com', apiToken: '' };
...
async function fetchStatusMulti(linkedinUrl, huntflowUrl) {
  ...
  try {
    ...
    if (normalized) q.set('linkedin_url', normalized);
    else if (huntNorm) q.set('huntflow_url', huntNorm);
    const res = await fetch(`${baseUrl}/api/v1/huntflow/linkedin-applicants/status-multi/?${q.toString()}`, {
      method: 'GET',
      headers: { 'Content-Type': 'application/json', 'Authorization': `Token ${apiToken}` },
    });
    const data = await res.json().catch(() => ({}));
    if (!res.ok) return { error: data?.message || data?.error || 'Ошибка ' + res.status };
    if (!data.success) return { error: data?.message || 'Ошибка' };
...
function isRejectionStatus(option) {
  if (!option || !option.dataset) return false;
