...
(async function () {
  ...
  /**
   * Расширенный парсинг данных резюме hh.ru/rabota.by
   * Извлекает зарплату, гражданство, разрешение на работу, релокацию, командировки, графики, навыки, «о себе»
   */
  function extractHHExtendedData() {
    var result = {};

    // Желаемая зарплата
    var salaryEl = document.querySelector('[data-qa="resume-block-salary"]');
    if (salaryEl) {
      var salaryText = (salaryEl.textContent || '').trim();
      var salaryMatch = salaryText.match(/([\d\s]+)/);
      result.salary = {
        amount: salaryMatch ? salaryMatch[1].replace(/\s/g, '') : '',
        currency: /USD|\$/i.test(salaryText) ? 'USD' : /EUR|€/i.test(salaryText) ? 'EUR' : 'RUB',
        text: salaryText
      };
    }

    // Гражданство
    var citizenshipEl = document.querySelector('[data-qa="resume-block-citizenship"]');
    if (citizenshipEl) result.citizenship = (citizenshipEl.textContent || '').trim();

    // Разрешение на работу
    var workPermitEl = document.querySelector('[data-qa="resume-block-work-permit"]');
    if (workPermitEl) {
      var permitItems = workPermitEl.querySelectorAll('[data-qa="resume-block-work-permit-item"]');
      result.workPermit = permitItems.length > 0
        ? Array.from(permitItems).map(function (el) { return (el.textContent || '').trim(); })
        : [(workPermitEl.textContent || '').trim()];
    }

    // Готовность к переезду
    var relocationEl = document.querySelector('[data-qa="resume-block-relocation"]');
    if (relocationEl) {
      var relText = (relocationEl.textContent || '').trim();
      result.relocation = {
        ready: /готов|yes|да/i.test(relText),
        text: relText
      };
    }

    // Готовность к командировкам
    var tripsEl = document.querySelector('[data-qa="resume-block-business-trip-readiness"]');
    if (tripsEl) result.businessTrips = (tripsEl.textContent || '').trim();

    // Занятость и график
    var employmentEl = document.querySelector('[data-qa="resume-block-employment"]');
    if (employmentEl) {
      var empItems = employmentEl.querySelectorAll('[data-qa="resume-block-employment-item"]');
      result.schedules = empItems.length > 0
        ? Array.from(empItems).map(function (el) { return (el.textContent || '').trim(); })
        : [(employmentEl.textContent || '').trim()];
    }

    // Навыки (теги)
    var skillsTable = document.querySelector('[data-qa="skills-table"]');
    if (skillsTable) {
      var tags = skillsTable.querySelectorAll('[data-qa="bloko-tag__text"]');
      result.skillsTags = Array.from(tags).map(function (el) { return (el.textContent || '').trim(); });
    }

    // О себе
    var aboutEl = document.querySelector('[data-qa="resume-block-skills"]');
    if (aboutEl) result.about = (aboutEl.textContent || '').trim();

    // Ключевые навыки
    var keySkillsEl = document.querySelector('[data-qa="resume-block-skills-content"]');
    if (keySkillsEl) {
      var ksTags = keySkillsEl.querySelectorAll('[data-qa="bloko-tag__text"]');
      result.keySkills = ksTags.length > 0
        ? Array.from(ksTags).map(function (el) { return (el.textContent || '').trim(); })
        : [(keySkillsEl.textContent || '').trim()];
    }
  ...
  g.__HRH__.extractHHExtendedData = extractHHExtendedData;
