...
(async function () {
  ...
  // Литерал ключа: не зависеть от порядка загрузки __HRH__ при чтении storage.onChanged
  var GDRIVE_APPS_KEY = 'hrhelper_gdrive_floating_apps';
  var DEFAULT_GDRIVE_APPS = (HRH.DEFAULT_GDRIVE_FLOATING_APPS && typeof HRH.DEFAULT_GDRIVE_FLOATING_APPS === 'object')
  ...
  function detectGdriveAppType() {
    try {
      var host = String(location.host || '').toLowerCase();
      var path = String(location.pathname || '').toLowerCase();
      if (host === 'drive.google.com') return 'drive';
      if (host === 'docs.google.com') {
        // Учёт URL с префиксом аккаунта: /u/0/spreadsheets/d/..., /u/0/document/d/...
        if (path.indexOf('/document/d/') !== -1) return 'docs';
        if (path.indexOf('/spreadsheets/d/') !== -1) return 'sheets';
        if (path.indexOf('/presentation/d/') !== -1) return 'slides';
        if (path.indexOf('/forms/') !== -1) return 'forms';
        return 'other';
      }
      return 'other';
  ...
  function coerceBool(x, fallback) {
  ...
  function normalizeGdriveApps(v) {
    var base = {};
    Object.keys(DEFAULT_GDRIVE_APPS).forEach(function (k) {
      base[k] = DEFAULT_GDRIVE_APPS[k];
    });
    if (!v || typeof v !== 'object') return base;
    Object.keys(base).forEach(function (k) {
      if (Object.prototype.hasOwnProperty.call(v, k)) {
        base[k] = coerceBool(v[k], base[k]);
      }
    });
    return base;
  }

  function isGdriveFloatingAllowedForCurrentPage(appsRaw) {
    var apps = normalizeGdriveApps(appsRaw);
    var t = detectGdriveAppType();
    return apps[t] !== false;
  }

  function removeExistingWidget() {
    try {
      var existing = document.querySelector('[data-hrhelper-gdrive-floating]');
      if (existing) existing.remove();
    } catch (_) {}
  }

  // Не выходим из IIFE до регистрации storage.onChanged: иначе смена настроек на другой
  // странице не убирает/не показывает виджет на уже открытой вкладке Google.
  ...
  async function refreshData() {
    ...
    if (infoData?.success) {
      ...
      try {
        const labelsArr = Array.isArray(candidateInfo.labels) ? candidateInfo.labels : [];
  ...
  async function initWidget() {
    try {
      var gate = await chrome.storage.sync.get({
        [ACTIVE_PAGES_KEY]: DEFAULT_ACTIVE_PAGES,
        [GDRIVE_APPS_KEY]: null,
      });
      var activeGate = gate[ACTIVE_PAGES_KEY] || DEFAULT_ACTIVE_PAGES;
      if (!activeGate.gdrive) { removeExistingWidget(); return; }
      if (!isGdriveFloatingAllowedForCurrentPage(gate[GDRIVE_APPS_KEY])) { removeExistingWidget(); return; }
    } catch (_) { return; }
  ...
  chrome.storage.onChanged.addListener((changes, area) => {
    ...
    if (area === 'sync' && (changes[GDRIVE_APPS_KEY] || changes[ACTIVE_PAGES_KEY])) {
      try {
        chrome.storage.sync.get({ [ACTIVE_PAGES_KEY]: DEFAULT_ACTIVE_PAGES, [GDRIVE_APPS_KEY]: null }, (data2) => {
          var active2 = data2[ACTIVE_PAGES_KEY] || DEFAULT_ACTIVE_PAGES;
          if (!active2.gdrive) { removeExistingWidget(); return; }
          if (!isGdriveFloatingAllowedForCurrentPage(data2[GDRIVE_APPS_KEY])) {
